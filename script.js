...
/* Una imagen propia para cada carrera. En las preguntas 2-5 se cambia
   automáticamente según las carreras que el estudiante va seleccionando. */
const imagenesCarrera = {
  ADM: 'img/carreras/adm.jpg',
  CIV: 'img/carreras/civ.png',
  CON: 'img/carreras/contaduria-general.png',
  GAS: 'img/carreras/gas.png',
  INF: 'img/carreras/inf.jpg',
  MEC: 'img/carreras/mec.png',
  SEC: 'img/carreras/sec.png',
  SIS: 'img/carreras/sis.png',
  TOP: 'img/carreras/top.jpg'
};
...
const enfoqueCarrera = document.getElementById('enfoqueCarrera');
const verCarrerasBtn = document.getElementById('verCarrerasBtn');
const volverResultadoBtn = document.getElementById('volverResultadoBtn');
const reiniciarDesdeCarrerasBtn = document.getElementById('reiniciarDesdeCarrerasBtn');
...
function cargarPregunta(){
  ...
  progresoTexto.textContent = `Pregunta ${indiceActual+1} de ${TOTAL_PREGUNTAS}`;
  progresoRelleno.style.width = ((indiceActual+1)/TOTAL_PREGUNTAS*100) + '%';

  /* Actualiza los círculos 1-5 como en el diseño de referencia. */
  document.querySelectorAll('.progreso-pasos .paso').forEach(paso=>{
    const numero = Number(paso.dataset.paso);
    paso.classList.toggle('activo', numero === indiceActual + 1);
    paso.classList.toggle('completado', numero < indiceActual + 1);
  });
  document.querySelectorAll('.progreso-pasos .linea-paso').forEach((linea, i)=>{
    linea.classList.toggle('completada', i < indiceActual);
  });

  preguntaTitulo.textContent = tituloParaPregunta();

  /* Primera pregunta: imagen general. Desde la segunda, la imagen
     corresponde a la carrera que está guiando el test. */
  let carreraEnfoque = null;
  if(indiceActual > 0 && candidatosActuales.length){
    carreraEnfoque = [...candidatosActuales].sort((a,b)=>puntajes[b]-puntajes[a])[0];
  }

  preguntaImagen.style.display = 'block';
  preguntaImagen.dataset.fallback = '0';
  preguntaImagen.src = carreraEnfoque ? imagenesCarrera[carreraEnfoque] : 'img/pregunta1.png';
  preguntaImagen.alt = carreraEnfoque
    ? `Imagen relacionada con ${carreras[carreraEnfoque].nombre}`
    : 'Imagen introductoria del test vocacional';
  // Si una imagen no carga, mantenemos siempre una imagen visible.
  preguntaImagen.onerror = function(){
    if (preguntaImagen.dataset.fallback !== '1') {
      preguntaImagen.dataset.fallback = '1';
      preguntaImagen.src = 'img/pregunta1.png';
      preguntaImagen.alt = 'Imagen del test vocacional';
      preguntaImagen.style.display = 'block';
    }
  };

  if(enfoqueCarrera){
    enfoqueCarrera.textContent = carreraEnfoque
      ? `Enfoque actual · ${carreras[carreraEnfoque].nombre}`
      : 'Exploraremos tus intereses para encontrar tu mejor perfil';
    enfoqueCarrera.classList.toggle('visible', Boolean(carreraEnfoque));
  }

  const carrerasMostrar = indiceActual === 0 ? orden : candidatosActuales;
  opcionesWrap.innerHTML = '';

  carrerasMostrar.forEach(key=>{
...
function crearResumenCarrera(key, recomendada=false){
  ...
  item.innerHTML = `
    <div class="carrera-mini-imagen-wrap">
      <img class="carrera-mini-imagen" src="${imagenesCarrera[key]}" alt="${c.nombre}" onerror="this.onerror=null;this.src='img/pregunta1.png';">
      ${recomendada ? '<span class="carrera-mini-badge">★ Recomendada</span>' : ''}
    </div>
    <div class="carrera-mini-body">
      <h4>${c.nombre}</h4>
      <p>${c.objetivo}</p>
      <button type="button" class="carrera-mini-info" data-key="${key}">Información</button>
    </div>
  `;
...
function lanzarCelebracion(){
  ...
  for(let i=0;i<18;i++){
    ...
    pieza.textContent = colores[i % colores.length];
    pieza.style.left = (Math.random()*100) + 'vw';
    pieza.style.animationDelay = (Math.random()*.35) + 's';
...
if(volverResultadoBtn){
  volverResultadoBtn.onclick = ()=>{
    sonido('seleccion');
    mostrarPantalla('pantallaResultado');
  };
}

if(reiniciarDesdeCarrerasBtn){
  reiniciarDesdeCarrerasBtn.onclick = ()=>{
    inputNombre.value = '';
    inputCelular.value = '';
    errorRegistro.textContent = '';
    mostrarPantalla('pantallaBienvenida');
